...
class GusiGun:
    def __init__(self):
        ...
        self.sub_url = "http://info.nec.go.kr/bizcommon/selectbox/selectbox_getSggCityCodeJson.json"
    ...
    def _fetch_content(self, *args, **kwargs):
        kwargs = {
            ...
            "statementId": kwargs.get("statementId", "CPRI03_#4"),
            "electionCode": kwargs.get("electionCode", 4),
            "cityCode": kwargs.get("cityCode", 1100),
            "sggCityCode": kwargs.get("sggCityCode", 4110100),
    ...
    def _fetch_sgg_code(self, *args, **kwargs):
        ...
        resp = requests.post(self.sub_url, data=kwargs)
        return resp
    ...
    def parse(self):
        ...
        for code, name in codes:
            total[code] = {}
            for sgg, sgg_name in self._get_sgg_codes(cityCode=code):
                total[code][sgg] = []
                candidates = self._parse_content(self._fetch_content(cityCode=code, sggCityCode=sgg), interested=[1, 3, 7, 9, 11, 13])
